...
fn main() {
    ...
    let mut for_pairs = vec![(0, 1), (2, 3), (4, 5)];
    for pair in &mut for_pairs {
        pair.0 += 1;
    }
    ...
    let mut iterator_numbers = vec![1, 2, 3, 4];
    let processed = iterator_numbers.iter().map(|x| x * 3).collect::<Vec<_>>();
